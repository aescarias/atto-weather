...
from atto_weather.data_fields import (AIR_QUALITY_FIELDS, CURRENT_WEATHER_FIELDS, 
                                      POINT16_COMPASS, US_EPA_INDEX, estimate_cloud_cover,
                                      estimate_uv_index, get_defra_index)
...
class CurrentWeatherWidget(WeatherFieldWidget):
    ...
    def update_details(self, current: dict[str, Any], *, _aqi: bool = True) -> None:
        self.set_label("feels_like", 
                       **get_temperature(current["feelslike_c"], current["feelslike_f"]))
        self.set_label("windchill",
                       **get_temperature(current["windchill_c"], current["windchill_f"]))
        self.set_label("heat_index",
                       **get_temperature(current["heatindex_c"], current["heatindex_f"]))
        self.set_label("dew_point",
                       **get_temperature(current["dewpoint_c"], current["dewpoint_f"]))
        self.set_label("wind_speed",
        ...
        self.set_label("cloud_cover", value=current["cloud"], summary=estimate_cloud_cover(current["cloud"]))
