...
from atto_weather.data_fields import (ASTRONOMY_FIELDS, DAILY_FORECAST_FIELDS,
                                      HOURLY_FORECAST_FIELDS, MOON_PHASE, estimate_uv_index)
...
class DailyForecastWidget(WeatherFieldWidget):
    ...
    def update_details(self, day_forecast: dict[str, Any]) -> None:
        ...
        self.set_label("min_max_temp", 
                       mintemp=min_temp["value"], minunit=min_temp["unit"],
                       maxtemp=max_temp["value"], maxunit=max_temp["unit"])
        self.set_label("max_wind",
                       **get_distance(day_forecast["maxwind_kph"], 
                                      day_forecast["maxwind_mph"], 
                                      speed=True))
        self.set_label("precipitation", 
